...
void CGame::Init(HWND hWnd, HINSTANCE hInstance)
{
	...
	if (hr != S_OK)
	{
		...
		DebugOut((wchar_t*)L"D3D10CreateDeviceAndSwapChain has done %s %d", _W(__FILE__), __LINE__);
	}
...
void CGame::Load(LPCWSTR gameFile)
{
	...
	TiXmlDocument doc("Scence.xml");

	if (doc.LoadFile()) {
		TiXmlElement* root = doc.RootElement();
		TiXmlElement* settings = root->FirstChildElement("SETTINGS");
		TiXmlElement* scences = root->FirstChildElement("SCENCES");
		TiXmlElement* textures = root->FirstChildElement("TEXTURES");

		// đọc settings
		TiXmlElement* start = settings->FirstChildElement("Start");
		if (start != nullptr)
			next_scene = atoi(start->Attribute("value"));
		else DebugOut(L"[ERROR] Unknown game setting start\n");
		DebugOut(L"Load scene start : &d", next_scene);

		// đọc scences
		for (TiXmlElement* node = scences->FirstChildElement("Scence"); node != nullptr; node = node->NextSiblingElement("Scence")) {
			int id = atoi(node->Attribute("id"));
			string path = node->Attribute("link");
		...
		for (TiXmlElement* node = textures->FirstChildElement("Texture"); node != nullptr; node = node->NextSiblingElement("Texture")) {
			int texID = atoi(node->Attribute("id"));
			wstring path = ToWSTR(node->Attribute("link"));

			DebugOut(L"load texture: %d/ %s\n", texID, path);
